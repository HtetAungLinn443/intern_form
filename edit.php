...
if (isset($_POST['submit']) && $_POST['form-sub'] == 1) {
    ...
    if (!$process_err) {
        if (!$image_upload) {
            $sql = "UPDATE INTO `user` SET " .
                ...
                "gender = '" . $form_gender . "' , " .
                "phone = '" . $form_phone . "' , " .
                "birth_of_date = '" . $form_date_of_birth . "' , " .
                "country_id = '" . $form_country . "' , " .
                "updated_at = '" . $date . 
                "' WHERE id = '" . $id . "'";
    ...
    if($result->num_rows <= 0){
        $error          = true;
        $error_msg      = "Something wrong. Please contact Administrator";
        ...
        while($row = $result->fetch_assoc()){
            ...
            $form_address            = htmlspecialchars($row['address']);
            $form_gender             = htmlspecialchars($row['gender']);
            $form_phone              = htmlspecialchars($row['phone']);
            $form_date_of_birth      = htmlspecialchars($row['birth_of_date']);
            $form_country            = htmlspecialchars($row['country_id']);
            require('include/include_user_job.php');
            while($job_row = $job_res->fetch_assoc()){
                array_push($job_types, $job_row['job_type_id']);
            }
        }
    }
}
    ...
    <div class="col-md-8">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <form action="<?php $base_url; ?>edit.php" method="POST" enctype="multipart/form-data">
